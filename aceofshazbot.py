...
def apply_script(protocol, connection, config):
    class AceofShazbotConnection(connection):
		...
		def on_chat(self, value, global_message):
			...
			if self.commands_global.has_key(upper):
				return connection.on_chat(self, self.commands_global[upper], True)

			if self.commands_team.has_key(upper):
				return connection.on_chat(self, self.commands_team[upper], False)
